const apiUrl =
  "https://opendata.brussels.be/api/explore/v2.1/catalog/datasets/streetart/records?limit=23";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setLanguage(language) {
    ...
    document.querySelector('a[href="Locatie.html"]').textContent =
      translations[language].location;
    document.querySelector('a[href="Favoriet.html"]').textContent =
...
async function fetchAndDisplayArtworks() {
  try {
    ...
    if (data && data.results) {
      ...
      if (searchTerm) {
        filteredData = filteredData.filter((record) => {
          ...
          const workName = record.name_of_the_work
            ? record.name_of_the_work.toLowerCase()
            : "";
          const artistName = record.nom_de_l_artiste
            ? record.nom_de_l_artiste.toLowerCase()
            : "";
          const adresSearch = record.adresse
            ? record.adresse.toLowerCase()
            : "";
          const descriptionSearch = record.explanation
            ? record.explanation.toLowerCase()
            : "";

          return (
            workName.includes(searchTerm) ||
            artistName.includes(searchTerm) ||
            adresSearch.includes(searchTerm) ||
            descriptionSearch.includes(searchTerm)
          );
        });
      }
      ...
      if (selectedType !== "all") {
        filteredData = filteredData.filter((record) => {
          if (selectedType === "metnaam") {
            ...
            return (
              record.name_of_the_work && record.name_of_the_work.trim() !== ""
            );
    ...
    dataContainer.innerHTML =
      "<p>Er is iets mis gegaan bij het laden van de data.</p>";
  }
}
